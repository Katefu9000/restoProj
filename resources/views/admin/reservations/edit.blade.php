        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex space-y-8 divide-y divide-gray-200 w-1/2 mt-10  bg-slate-700 rounded">
                    <form method="POST" action=" {{ route('admin.reservations.update', $reservation->id) }}" class="m-2">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="text" id="no_tel" name="no_tel" value="{{$reservation->no_tel}}" class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                            </div>
                            @error('no_tel')
                                <div class="text-sm text-red-500">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                            <input type="dateTime-local" id="res_date" name="res_date" value="{{ $reservation->res_date->format('Y-m-d\TH:i:s') }}" class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base">
                            </div>
                            @error('res_date')
                                <div class="text-sm text-red-500">{{ $message }}</div>
                            @enderror
